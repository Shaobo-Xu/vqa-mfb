...
def main():
    ...
    for it in range(restore_iter, config.MAX_ITERATIONS + 1):
        ...
        if it % config.VALIDATE_INTERVAL == 0 and it != restore_iter:
            ...
            test_loss, acc_overall, acc_per_ques, acc_per_ans = exec_validation(config.TEST_GPU_ID, 'val', model_name,
                                                                                it=it, folder=folder)
            caffe.set_device(config.TRAIN_GPU_ID)
            print
            'Test loss:', test_loss
            ...
            print
            'Best accuracy of', results[best_result_idx][3], 'was at iteration', results[best_result_idx][0]
