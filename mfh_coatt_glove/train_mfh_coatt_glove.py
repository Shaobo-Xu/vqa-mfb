...
from utils import exec_validation
import config
import time
...
def mfb_coatt(mode, batchsize, T, question_vocab_size, folder):
    ...
    n.qatt_conv2 = L.Convolution(n.qatt_relu, kernel_size=1, stride=1, num_output=config.NUM_QUESTION_GLIMPSE, pad=0,
                                 weight_filler=dict(type='xavier'))
    n.qatt_reshape = L.Reshape(n.qatt_conv2, reshape_param=dict(shape=dict(
        dim=[-1, config.NUM_QUESTION_GLIMPSE, config.MAX_WORDS_IN_QUESTION, 1])))  # N*NUM_QUESTION_GLIMPSE*15
    ...
    n.i_feat_resh = L.Reshape(n.img_feature,
                              reshape_param=dict(shape=dict(dim=[0, -1, config.IMG_FEAT_WIDTH, config.IMG_FEAT_WIDTH])))
    ...
    n.iatt_iq_resh2 = L.Reshape(n.iatt_iq_droped,
                                reshape_param=dict(shape=dict(dim=[-1, config.JOINT_EMB_SIZE, 196, 1])))
    ...
    n.iatt_conv2 = L.Convolution(n.iatt_relu, kernel_size=1, stride=1, num_output=config.NUM_IMG_GLIMPSE, pad=0,
                                 weight_filler=dict(type='xavier'))
    n.iatt_resh = L.Reshape(n.iatt_conv2,
                            reshape_param=dict(shape=dict(dim=[-1, config.NUM_IMG_GLIMPSE, config.IMG_FEAT_SIZE])))
    n.iatt_softmax = L.Softmax(n.iatt_resh, axis=2)
    n.iatt_softmax_resh = L.Reshape(n.iatt_softmax, reshape_param=dict(
        shape=dict(dim=[-1, config.NUM_IMG_GLIMPSE, config.IMG_FEAT_WIDTH, config.IMG_FEAT_WIDTH])))
    ...
    n.mfb_iq_o2_resh = L.Reshape(n.mfb_iq_o2_drop,
                                 reshape_param=dict(shape=dict(dim=[-1, 1, config.MFB_OUT_DIM, config.MFB_FACTOR_NUM])))
    ...
    n.mfb_iq_o3_resh = L.Reshape(n.mfb_iq_o3_drop,
                                 reshape_param=dict(shape=dict(dim=[-1, 1, config.MFB_OUT_DIM, config.MFB_FACTOR_NUM])))
...
def main():
    folder = 'mfh_coatt_glove_q%dv%d_%s' % (
    config.NUM_QUESTION_GLIMPSE, config.NUM_IMG_GLIMPSE, config.TRAIN_DATA_SPLITS)
    ...
    print
    'question vocab size:', len(question_vocab)
    print
    'answer vocab size:', len(answer_vocab)
    ...
    for it in range(restore_iter, config.MAX_ITERATIONS + 1):
        ...
        if it % config.PRINT_INTERVAL == 0 and it != 0:
            ...
            print
            'Train loss:', c_mean_loss, ' Elapsed seconds:', elapsed
            start = time.clock()
